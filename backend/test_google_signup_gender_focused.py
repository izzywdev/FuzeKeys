...
class GenderDropdownTest:
    ...
    def analyze_gender_dropdown(self):
        ...
        for i, combo in enumerate(comboboxes):
            ...
            print(
                f"  Combobox {i}: text='{combo_text}', id='{combo_id}', aria-label='{combo_aria_label}'"
            )
            print(
                f"    class='{combo_class[:100]}...' "
                if len(combo_class) > 100
                else f"    class='{combo_class}'"
            )
        ...
        if not gender_combobox:
            ...
            if len(comboboxes) >= 2:
                gender_combobox = comboboxes[
                    1
                ]  # Usually month is first, gender is second
    ...
    def try_select_gender_option(self, target_gender_value="1"):
        ...
        for i, option in enumerate(options):
            ...
            print(
                f"  Option {i}: text='{option_text}', data-value='{option_data_value}', visible={option_visible}, enabled={option_enabled}"
            )
            print(
                f"    class='{option_class[:80]}...' "
                if len(option_class) > 80
                else f"    class='{option_class}'"
            )
        ...
        if not target_option:
            ...
            if gender_options:
                ...
                print(
                    f"🔄 Using first available option: data-value='{target_option['data_value']}'"
                )
    ...
    async def test_gender_dropdown_focused(self):
        ...
        try:
            ...
            month_combobox = self.driver.find_element(
            ...
            if success:
                ...
                if next_button:
                    ...
                    if "username" in current_url or "gmail" in current_url:
                        ...
                        return {
                            "success": False,
                            "status": "unexpected_page_after_gender",
                        }
                else:
                    print("❌ Could not find Next button after gender selection")
                    return {
                        "success": False,
                        "error": "Next button not found after gender selection",
                    }
            else:
