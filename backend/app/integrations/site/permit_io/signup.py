...
from .models import PermitIOAccountInfo, PermitIOCredentials, PermitIOResult
...
class SignupAutomation:
    ...
    async def take_screenshot(self, name: str) -> str:
        ...
        await self.page.screenshot(
            path=screenshot_path, **self.config.SCREENSHOT_OPTIONS
        )
        return screenshot_path

    async def fill_form_field(
        self, selectors: list, value: str, field_name: str
    ) -> bool:
        """Try to fill a form field using multiple selectors."""
        for selector in selectors:
            try:
                element = await self.page.wait_for_selector(
                    selector, timeout=self.config.get_timeout("element")
                )
                if element:
    ...
    async def check_result(self) -> PermitIOResult:
        ...
        for indicator in self.config.SUCCESS_INDICATORS:
            if (
                ...
                or indicator.lower() in current_url.lower()
            ):
    ...
    async def create_account(self, credentials: PermitIOCredentials) -> PermitIOResult:
        ...
        try:
            ...
            if (
                "signup" not in self.page.url.lower()
                and "register" not in self.page.url.lower()
            ):
                signup_selectors = [
                    'a[href*="signup"]',
                    'a[href*="register"]',
            ...
            if not await self.fill_form_field(
                self.config.get_selectors("email"), credentials.email, "email"
            ):
                return PermitIOResult(
                    success=False,
            ...
            # Fill password field
            if not await self.fill_form_field(
                self.config.get_selectors("password"), credentials.password, "password"
            ):
                return PermitIOResult(
                    success=False,
            ...
            if credentials.phone:
                await self.fill_form_field(
                    self.config.get_selectors("phone"), credentials.phone, "phone"
                )
            ...
            return PermitIOResult(
                success=False, message="Signup failed with exception", error=str(e)
            )


# Convenience function
async def create_account(
    credentials: PermitIOCredentials, headless: bool = True
) -> PermitIOResult:
    """
