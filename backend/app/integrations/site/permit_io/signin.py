...
class SigninAutomation:
    ...
    async def extract_session_info(self) -> Optional[PermitIOSessionInfo]:
        ...
        try:
            ...
            session_id = (
                cookie_dict.get("session_id")
                or cookie_dict.get("sessionId")
                or f"session_{hash(current_url)}"
            )
    ...
    async def check_signin_result(self) -> PermitIOResult:
        ...
        if any(
            ...
            for indicator in ["dashboard", "app", "workspace"]
        ):
        ...
        if (
            current_url != self.config.SIGNIN_URL
    ...
    async def authenticate(self, email: str, password: str) -> PermitIOResult:
        ...
        try:
            ...
            if (
                "signin" not in self.page.url.lower()
            ...
            if not await self.fill_form_field(
                self.config.get_selectors("email"), email, "email"
            ):
                return PermitIOResult(
                    success=False,
            ...
            # Fill password field
            if not await self.fill_form_field(
                self.config.get_selectors("password"), password, "password"
            ):
                return PermitIOResult(
                    success=False,
            ...
            return PermitIOResult(
                success=False, message="Signin failed with exception", error=str(e)
            )
