...
from .models import PermitIOApiKey, PermitIOResult
    ...
    async def create_api_key_form(self, key_name: str) -> Optional[str]:
        ...
        try:
            ...
            for selector in self.config.API_KEY_SELECTORS["key_name"]:
                try:
                    name_field = await self.page.wait_for_selector(
                        selector, timeout=5000
                    )
                    if name_field:
                ...
                try:
                    ...
                    for element in elements:
                        ...
                        if text and ("permit_" in text or "pk_" in text):
                            api_keys.append({"text": text.strip(), "element": element})
                except:
                    continue
...
async def create_key(
    email: str, password: str, key_name: str = "FuzeKeys", headless: bool = True
) -> PermitIOResult:
    """
