...
from .models import GoogleSignupConfig, GoogleSignupData, GoogleSignupResult
...
class GoogleSignupService:
    ...
    async def signup_with_identity(self, identity: Identity) -> GoogleSignupResult:
        ...
        try:
            ...
            logger.error(
                f"Error during Google signup with identity {identity.id}: {str(e)}"
            )
            return GoogleSignupResult(
                success=False, error_message=f"Identity conversion error: {str(e)}"
            )
    ...
    async def _identity_to_signup_data(self, identity: Identity) -> GoogleSignupData:
        ...
        # Decrypt fields
        first_name = (
            decrypt_field(identity.encrypted_first_name)
            if identity.encrypted_first_name
            else None
        )
        last_name = (
            decrypt_field(identity.encrypted_last_name)
            if identity.encrypted_last_name
            else None
        )
        email = (
            decrypt_field(identity.encrypted_email)
            if identity.encrypted_email
            else None
        )
        phone = (
            decrypt_field(identity.encrypted_phone)
            if identity.encrypted_phone
            else None
        )
        dob = (
            decrypt_field(identity.encrypted_date_of_birth)
            if identity.encrypted_date_of_birth
            else None
        )
        ...
        username = self._generate_username(
            first_name, last_name, email, identity.preferred_username_pattern
        )
    ...
    async def signup(self, data: GoogleSignupData) -> GoogleSignupResult:
        ...
        while attempts < self.config.retry_attempts:
            try:
                ...
                try:
                    if data.birth_date:
                        month_field = self._wait_and_find_element(
                            By.ID, "month", timeout=5
                        )
                        ...
                        # Gender (if present)
                        if data.gender:
                            gender_field = self._wait_and_find_element(
                                By.ID, "gender", timeout=3
                            )
                ...
                try:
                    ...
                    username_field = self._wait_and_find_element(
                        By.NAME, "Username", timeout=10
                    )
                    ...
                    error_elements = self.driver.find_elements(
                        By.CSS_SELECTOR, "[data-error-id]"
                    )
                    if error_elements:
                        # Username taken, try suggested ones
                        suggested_usernames = self.driver.find_elements(
                            By.CSS_SELECTOR, "[data-value]"
                        )
                ...
                confirm_password_field = self._wait_and_find_element(
                ...
                if result.success:
                    ...
                    try:
                        ...
                        if email_elements:
                            result.account_email = email_elements[0].get_attribute(
                                "data-email"
                            )
                        else:
    ...
    async def _handle_verification(self, data: GoogleSignupData) -> GoogleSignupResult:
        ...
        try:
            ...
            if (
                ...
                and not data.skip_phone_verification
            ):
            ...
            recovery_email_elements = self.driver.find_elements(
            ...
            try:
                ...
                if not agree_buttons:
                    agree_buttons = self.driver.find_elements(
                        By.XPATH, "//span[contains(text(), 'Create account')]"
                    )
                ...
                if agree_buttons:
                    ...
                    if (
                        "myaccount.google.com" in self.driver.current_url
                        or "welcome" in self.driver.current_url.lower()
                    ):
