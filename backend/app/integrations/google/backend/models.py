...
class GoogleSignupData(BaseModel):
    ...
    phone_number: Optional[str] = Field(
        None, description="Phone number for account recovery"
    )
    recovery_email: Optional[str] = Field(None, description="Recovery email address")

    # Personal information
    birth_date: Optional[date] = Field(
        None, description="Date of birth (required for some regions)"
    )
    ...
    def validate_username(cls, v):
        ...
        if ".." in v or v.startswith(".") or v.endswith("."):
            raise ValueError(
                "Username cannot have consecutive dots or start/end with dots"
            )
        return v.lower()
