...
class CaptchaSolverService:
    ...
    async def _solve_text_captcha(
        ...
        try:
            ...
            image_base64 = await self._prepare_image(
                challenge.image_data, challenge.image_url
            )
            if not image_base64:
                return None
    ...
    def _parse_solution_response(self, content: str) -> Optional[CaptchaSolution]:
        ...
        try:
            ...
            return CaptchaSolution(
                solution=solution, confidence=confidence, reasoning=reasoning
            )
...
class CaptchaDetector:
    ...
    async def detect_captcha(page: Page) -> Optional[CaptchaChallenge]:
        ...
        try:
            ...
            recaptcha = await page.query_selector(
                ".g-recaptcha, #recaptcha, [data-recaptcha]"
            )
            if recaptcha:
            ...
            if captcha_inputs:
                return await CaptchaDetector._analyze_text_captcha(
                    page, captcha_inputs[0]
                )
    ...
    async def _analyze_recaptcha(
        ...
        try:
            ...
            challenge_img = await page.query_selector(
                ".rc-image-tile-wrapper img, .rc-imageselect-payload img"
            )
            if challenge_img:
                img_src = await challenge_img.get_attribute("src")
                question_elem = await page.query_selector(
    ...
    @staticmethod
    async def _analyze_image_captcha(
        page: Page, img_element: ElementHandle
    ) -> Optional[CaptchaChallenge]:
        ...
        try:
            ...
            question_elem = (
                await parent.query_selector("label, .question, .instruction")
                if parent
                else None
            )
            question = (
                await question_elem.text_content()
                if question_elem
                else "Enter the text shown in the image"
            )
    ...
    @staticmethod
    async def _analyze_text_captcha(
        page: Page, input_element: ElementHandle
    ) -> Optional[CaptchaChallenge]:
...
class CaptchaHandler:
    ...
    async def handle_page_captcha(self, page: Page) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Found {challenge.challenge_type} captcha: {challenge.question}"
            )
            ...
            if not solution or solution.confidence < self.solver.confidence_threshold:
                logger.warning(
                    f"Could not solve captcha with sufficient confidence: {solution.confidence if solution else 0}"
                )
                return False

            logger.info(
                f"Solved captcha: {solution.solution} (confidence: {solution.confidence})"
            )
    ...
    async def _apply_solution(
        self, page: Page, challenge: CaptchaChallenge, solution: CaptchaSolution
    ) -> bool:
        ...
        try:
            if challenge.challenge_type == "text":
                ...
                if solution.solution == "click_checkbox":
                    ...
                    squares = (
                        solution.solution.replace("SQUARES:", "").strip().split(",")
                    )
                ...
                submit_button = await page.query_selector(
                    '.hcaptcha-submit, [aria-label="Submit"]'
                )
                if submit_button:
