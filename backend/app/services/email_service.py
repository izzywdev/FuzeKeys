...
class EmailVerificationService:
        ...
        try:
            ...
            if result == "OK" and message_ids[0]:
                for message_id in message_ids[0].split():
                    verification_email = await self._process_email(
                        mail, message_id, config
                    )
                    if verification_email:
                        await self._handle_verification_email(
                            verification_email, config
                        )
    ...
    async def _process_email(
        self, mail, message_id: bytes, config: EmailConfig
    ) -> Optional[VerificationEmail]:
        ...
        try:
            ...
            if self._is_verification_email(sender, subject, body):
                ...
                verification_email.verification_link = self._extract_verification_link(
                    body
                )
                verification_email.verification_code = self._extract_verification_code(
                    body
                )
                verification_email.website = self._identify_website(
                    sender, subject, body
                )
    ...
    def _is_verification_email(self, sender: str, subject: str, body: str) -> bool:
        ...
        keyword_count = sum(
            1 for keyword in verification_keywords if keyword in text_to_check
        )
    ...
    def _extract_verification_link(self, body: str) -> Optional[str]:
        ...
        for link in general_links:
            if any(
                ...
                for keyword in ["verify", "confirm", "activate", "token", "validation"]
            ):
                return link.rstrip(".,;!?)")
    ...
    async def _handle_verification_email(
        self, verification_email: VerificationEmail, config: EmailConfig
    ):
        """Handle a detected verification email"""
        logger.info(
            f"Found verification email from {verification_email.sender} for {verification_email.website}"
        )
    ...
    async def _store_verification_email(
        self, verification_email: VerificationEmail, email_account: str
    ):
        """Store verification email in database"""
        # TODO: Implement database storage
        logger.info(
            f"Storing verification email: {verification_email.website} - {verification_email.verification_link}"
        )
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(
                    verification_email.verification_link, headers=headers
                ) as response:
                    if response.status == 200:
                        logger.info(
                            f"Successfully verified email for {verification_email.website}"
                        )
                    else:
                        logger.warning(
                            f"Verification link returned status {response.status}"
                        )
...
def create_email_config(
    email_address: str, password: str, provider: str = None
) -> EmailConfig:
    ...
    if provider and provider.lower() in COMMON_EMAIL_CONFIGS:
        ...
        return EmailConfig(
            email_address=email_address, password=password, **config_data
        )
    ...
    return EmailConfig(email_address=email_address, password=password, **config_data)
