...
from .captcha_service import CaptchaHandler, CaptchaSolverService
from .email_service import EmailConfig, EmailVerificationService, VerificationEmail
    ...
    async def start(self):
        ...
        try:
            ...
            if self.email_configs:
                ...
                logger.info(
                    f"Started email monitoring for {len(self.email_configs)} accounts"
                )
        ...
        while self.running:
            try:
                ...
                if success:
                    ...
                    task.retry_count += 1
                    logger.warning(
                        f"Task failed: {task.task_id} (attempt {task.retry_count})"
                    )
                ...
                for job in self.automation_jobs.values():
                    if job.status == "awaiting_verification" and job.verification_email:
                        ...
                        if job.verification_email.received_date:
                            time_diff = (
                                datetime.now() - job.verification_email.received_date
                            )
        ...
        try:
            ...
            if result.get("success"):
                if result.get("verification_required"):
                    ...
                    logger.info(
                        f"Signup completed, awaiting email verification: {job_id}"
                    )
                else:
    ...
    async def _execute_signup(self, job: AutomationJob) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                # Submit form
                await page.click(
                    'button[type="submit"], input[type="submit"], .signup-button'
                )
            ...
            if verification_email.get("verification_link"):
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.get(
                        verification_email["verification_link"]
                    ) as response:
...
async def initialize_background_manager(
    openai_api_key: str, encryption_manager: EncryptionManager
):
    """Initialize the global background service manager"""
    global background_manager
    if not background_manager:
        background_manager = BackgroundServiceManager(
            openai_api_key, encryption_manager
        )
...
@asynccontextmanager
async def background_service_context(
    openai_api_key: str, encryption_manager: EncryptionManager
):
    """Context manager for background services"""
    manager = None
    try:
        manager = await initialize_background_manager(
            openai_api_key, encryption_manager
        )
        yield manager
    finally:
