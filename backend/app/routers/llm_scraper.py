...
async def improve_scraper(request: ImproveScraperRequest):
    ...
    try:
        ...
        latest_scraper = code_generator.get_latest_scraper(
            request.site_name, request.action_type
        )
        ...
        # Improve scraper
        result = await code_generator.improve_scraper(
            latest_scraper, request.execution_result
        )
...
async def delete_scraper(site_name: str, action_type: str):
    ...
    try:
        ...
        if scraper_id not in code_generator.generation_history:
            raise HTTPException(
                status_code=404, detail=f"Scraper not found: {site_name} {action_type}"
            )
...
async def debug_scraper(
    site_name: str, action_type: str, error: str, context: Dict[str, Any]
):
