...
async def get_site_by_name(
    site_name: str, db: AsyncSession = Depends(get_async_session)
):
...
async def create_site(
    site_data: SiteCreate, db: AsyncSession = Depends(get_async_session)
):
    """Create a new site."""
    try:
...
async def update_site(
    site_id: int, site_data: SiteUpdate, db: AsyncSession = Depends(get_async_session)
):
    ...
    try:
        ...
        for row_num, row in enumerate(csv_data, start=2):  # Start at 2 for header
            try:
                ...
                try:
                    anti_bot_techniques = json.loads(
                        row.get("anti_bot_techniques", "[]")
                    )
                ...
                site_data = {
                    ...
                    "signup_difficulty": DifficultyLevel(
                        row.get("signup_difficulty", "medium")
                    ),
                    "signin_difficulty": DifficultyLevel(
                        row.get("signin_difficulty", "easy")
                    ),
                    "apikey_difficulty": DifficultyLevel(
                        row.get("apikey_difficulty", "medium")
                    ),
                    "requires_email_verification": row.get(
                        "requires_email_verification", "true"
                    ).lower()
                    == "true",
                    "requires_phone_verification": row.get(
                        "requires_phone_verification", "false"
                    ).lower()
                    == "true",
                    "requires_sms_verification": row.get(
                        "requires_sms_verification", "false"
                    ).lower()
                    == "true",
                    "requires_authenticator": row.get(
                        "requires_authenticator", "false"
                    ).lower()
                    == "true",
                    ...
                    "signup_status": ImplementationStatus(
                        row.get("signup_status", "not_started")
                    ),
                    "signin_status": ImplementationStatus(
                        row.get("signin_status", "not_started")
                    ),
                    "apikey_status": ImplementationStatus(
                        row.get("apikey_status", "not_started")
                    ),
                    "priority": int(row.get("priority", 50)),
                    "estimated_hours": int(row["estimated_hours"])
                    if row.get("estimated_hours")
                    else None,
                    "has_official_api": row.get("has_official_api", "true").lower()
                    == "true",
