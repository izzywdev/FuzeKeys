...
def _get_cipher() -> Fernet:
    ...
    try:
        ...
        logger.error(
            "Refusing credential crypto operation: invalid Fernet key: %s", exc
        )
        log_security_event(
            "credential_crypto_unavailable",
...
class CredentialRequest(BaseModel):
    ...
    identity_id: int = Field(
        ..., description="ID of the identity to generate credentials for", example=1
    )
    site_name: str = Field(
        ..., description="Name of the target website", example="github"
    )
    action_type: str = Field(
        ..., description="Type of action being performed", example="signup"
    )
    credential_types: List[str] = Field(
        ...,
        description="List of credential types needed",
...
class AccountCredentialRequest(BaseModel):
    ...
    identity_id: int = Field(
        ..., description="ID of the identity that owns the account", example=1
    )
    account_id: int = Field(
        ..., description="ID of the account to retrieve credentials for", example=1
    )
    credential_types: List[str] = Field(
        ...,
        description="List of credential types to retrieve",
...
class CredentialResponse(BaseModel):
    ...
    metadata: Dict[str, Any] = Field(
        ..., description="Request metadata and tracking information"
    )
...
class AccountCredentialResponse(BaseModel):
    ...
    last_used: Optional[datetime] = Field(
        None, description="Timestamp when credentials were last accessed"
    )
...
class CredentialUpdate(BaseModel):
    ...
    identity_id: int = Field(
        ..., description="ID of the identity that owns the account"
    )
    account_id: int = Field(
        ..., description="ID of the account to store credentials for"
    )
    credentials: Dict[str, str] = Field(..., description="Credentials to store")
    metadata: Optional[Dict[str, Any]] = Field(
        None, description="Additional metadata about the credentials"
    )
...
class ValidationResult(BaseModel):
    ...
    warnings: List[str] = Field(
        ..., description="List of warnings about the credentials"
    )
...
def generate_credentials_for_identity(
    identity: Identity, site_name: str, action_type: str
) -> Dict[str, str]:
    ...
    credentials["last_name"] = (
        identity.name.split()[-1] if len(identity.name.split()) > 1 else "Name"
    )
...
async def request_identity_credentials(
    ...
    try:
        ...
        credentials = generate_credentials_for_identity(
            identity, request.site_name, request.action_type
        )
...
async def request_account_credentials(
    ...
    try:
        ...
        logger.info(
            f"Retrieving credentials for account {account.website_name} by {service_name}"
        )
...
async def store_account_credentials(
    ...
    try:
        ...
        logger.info(
            f"Storing credentials for account {account.website_name} by {service_name}"
        )
...
async def validate_credentials(
    ...
    try:
        ...
        rules = validation_rules.get(
            request.site_name.lower(), validation_rules["default"]
        )
