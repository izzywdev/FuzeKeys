...
class EmailMonitor:
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self.imap_client.login(self.credentials.email, self.credentials.password)
    ...
    async def setup_monitoring(
        ...
        logger.info(
            f"Setup email monitoring {monitor_id} for patterns: {sender_patterns}"
        )
    ...
    async def _check_for_new_emails(self):
        ...
        try:
            ...
            since_time = (datetime.utcnow() - timedelta(minutes=10)).strftime(
                "%d-%b-%Y"
            )
    ...
    async def _check_email_against_monitors(self, email_match: EmailMatch):
        ...
        for monitor_id, monitor in self.monitors.items():
            ...
            if sender_match or subject_match:
                ...
                logger.info(
                    f"Email match found for monitor {monitor_id}: {email_match.subject}"
                )
...
def get_email_credentials_for_provider(
