...
class OpenAIClient(BaseLLMClient):
        ...
        try:
            response = await self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are an expert web automation engineer. Generate clean, production-ready Python code for web scraping and automation.",
                    },
    ...
    def _calculate_cost(
        self, total_tokens: int, input_tokens: int, output_tokens: int
    ) -> float:
...
class LLMClientManager:
    ...
    def _initialize_clients(self):
        ...
        if local_url:
            self.clients[LLMProvider.LOCAL] = LocalLLMClient(
                base_url=local_url, model=os.getenv("LOCAL_LLM_MODEL", "codellama")
            )
    ...
    async def generate_code(
        self, prompt: str, provider: Optional[LLMProvider] = None, **kwargs
    ) -> LLMResponse:
