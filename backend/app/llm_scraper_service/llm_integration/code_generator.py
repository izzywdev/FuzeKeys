...
from .llm_client import LLMResponse, llm_manager
...
class ScraperCodeGenerator:
    ...
    async def generate_initial_scraper(self, site_info: SiteInfo) -> GenerationResult:
        ...
        try:
            logger.info(
                f"Generating initial scraper for {site_info.name} {site_info.action_type}"
            )
            ...
            llm_response = await llm_manager.generate_code(
                prompt=prompt, temperature=0.1, max_tokens=4000
            )
            ...
            return GenerationResult(
                success=True, scraper_code=scraper_code, llm_response=llm_response
            )
    ...
    async def improve_scraper(
        self, scraper_code: ScraperCode, execution_result: Dict[str, Any]
    ) -> GenerationResult:
        """Improve existing scraper based on execution results"""
        try:
            logger.info(
                f"Improving scraper {scraper_code.site_name} v{scraper_code.version}"
            )
            ...
            if execution_result.get("success", False):
                ...
                return GenerationResult(success=True, scraper_code=scraper_code)

            # Build iteration prompt
            prompt = self.templates.build_iteration_prompt(
                previous_code=scraper_code.content, execution_result=execution_result
            )
            ...
            return GenerationResult(
                success=True, scraper_code=improved_code, llm_response=llm_response
            )
    ...
    async def debug_scraper(
        self, scraper_code: ScraperCode, error: str, context: Dict[str, Any]
    ) -> GenerationResult:
        """Debug specific issues in scraper code"""
        try:
            logger.info(
                f"Debugging scraper {scraper_code.site_name} v{scraper_code.version}"
            )

            # Build debugging prompt
            prompt = self.templates.build_debugging_prompt(
                code=scraper_code.content, error=error, context=context
            )
            ...
            return GenerationResult(
                success=True, scraper_code=debugged_code, llm_response=llm_response
            )
    ...
    def get_latest_scraper(
        self, site_name: str, action_type: str
    ) -> Optional[ScraperCode]:
    ...
    def get_generation_stats(self) -> Dict[str, Any]:
        ...
        total_versions = sum(
            len(versions) for versions in self.generation_history.values()
        )
                ...
                for filename in os.listdir(site_dir):
                    if filename.endswith("_metadata.json"):
                        ...
                        if os.path.exists(code_path):
                            ...
                            # Add to history
                            scraper_id = (
                                f"{scraper_code.site_name}_{scraper_code.action_type}"
                            )
                            if scraper_id not in self.generation_history:
                                self.generation_history[scraper_id] = []
                            self.generation_history[scraper_id].append(scraper_code)
