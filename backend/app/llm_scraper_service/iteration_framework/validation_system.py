...
class ScraperValidator:
        ...
        return {
            ...
            "apikey_creation": [
                ...
                TestCase(
                    ...
                    test_data={"key_name": "Limited Key", "permissions": ["read"]},
                    expected_outcome="success",
                    timeout=240,
                    weight=1.0,
                ),
    ...
    async def validate_scraper(
        ...
        test_cases = custom_test_cases or self._get_default_test_cases(
            scraper_code.action_type
        )
        ...
        logger.info(
            f"Starting validation run {run_id} with {len(test_cases)} test cases"
        )
        ...
        try:
            ...
            for test_case in test_cases:
                ...
                logger.info(
                    f"Test case {test_case.name} completed: {'PASS' if result.success else 'FAIL'}"
                )

            # Calculate success metrics
            validation_run.success_rate = self._calculate_success_rate(validation_run)
            validation_run.overall_success = (
                validation_run.success_rate >= min_success_rate
            )
            validation_run.status = (
                ValidationStatus.PASSED
                if validation_run.overall_success
                else ValidationStatus.FAILED
            )
            validation_run.completed_at = datetime.utcnow()

            logger.info(
                f"Validation run {run_id} completed: {validation_run.status.value} (success rate: {validation_run.success_rate:.2%})"
            )
    ...
    def _validate_specific_outcome(
        self, result: ExecutionResult, expected: str
    ) -> bool:
        ...
        if expected.startswith("contains:"):
            ...
            return any(
                code_pattern in log for log in [result.logs, str(result.return_data)]
            )

        elif expected.startswith("redirect:"):
            # Check for specific redirect URL
            redirect_url = expected.replace("redirect:", "").strip()
            return redirect_url in result.logs or redirect_url in str(
                result.return_data
            )
    ...
    async def run_regression_tests(
        self, scraper_code: ScraperCode, previous_results: List[ValidationRun]
    ) -> ValidationRun:
        ...
        logger.info(
            f"Running regression tests with {len(regression_test_cases)} test cases"
        )
    ...
    async def run_stress_tests(self, scraper_code: ScraperCode) -> ValidationRun:
        ...
        logger.info(
            f"Running stress tests with {len(stress_test_cases)} concurrent test cases"
        )
    ...
    async def continuous_validation(
        self, scraper_code: ScraperCode, interval_hours: int = 24
    ):
        """Run continuous validation checks"""

        logger.info(
            f"Starting continuous validation for {scraper_code.site_name}_{scraper_code.action_type}"
        )
        ...
        while True:
            try:
                ...
                if not validation_run.overall_success:
                    logger.warning(
                        f"Continuous validation failed for {scraper_code.site_name}_{scraper_code.action_type}: {validation_run.success_rate:.2%}"
                    )
