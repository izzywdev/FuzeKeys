...
from ..llm_integration.code_generator import (
    GenerationResult,
    ScraperCode,
    code_generator,
)
from ..scraper_engine.executor import ExecutionResult, scraper_executor
from .validation_system import ValidationRun, ValidationStatus, validator
...
class ScraperImprovementEngine:
    ...
    async def improve_scraper_iteratively(
        ...
        for iteration in range(max_iter):
            ...
            logger.info(
                f"Starting improvement iteration {iteration + 1}/{max_iter} for {scraper_id}"
            )

            try:
                # Validate current scraper
                validation_result = await validator.validate_scraper(
                    scraper_code=current_scraper, min_success_rate=target_success_rate
                )
                ...
                if (
                    ...
                    and validation_result.success_rate >= target_success_rate
                ):
                    improvement_iter.success = True
                    improvement_iter.completed_at = datetime.utcnow()

                    logger.info(
                        f"Scraper {scraper_id} meets quality standards: {validation_result.success_rate:.2%}"
                    )
                ...
                if iteration < max_iter - 1:
                    logger.info(
                        f"Improving scraper {scraper_id} (success rate: {validation_result.success_rate:.2%})"
                    )

                    # Generate improvement
                    improvement_result = await self._generate_improvement(
                        current_scraper, validation_result
                    )
                    ...
                    if improvement_result.success:
                        ...
                        logger.error(
                            f"Failed to generate improvement: {improvement_result.error_message}"
                        )
                        break
        ...
        logger.warning(
            f"Failed to improve scraper {scraper_id} to target quality after {max_iter} iterations"
        )

        return improvement_iter

    async def _generate_improvement(
        self, scraper_code: ScraperCode, validation_result: ValidationRun
    ) -> GenerationResult:
    ...
    def _extract_html_context(self, failed_results: List[ExecutionResult]) -> str:
        ...
        html_contexts = [
            result.html_context for result in failed_results if result.html_context
        ]
        if html_contexts:
            # Return the most recent HTML context (truncated)
            return (
                html_contexts[-1][:2000] + "..."
                if len(html_contexts[-1]) > 2000
                else html_contexts[-1]
            )
        return ""
    ...
    def _combine_logs(self, failed_results: List[ExecutionResult]) -> str:
        ...
        for result in failed_results:
            if result.logs:
                all_logs.append(
                    f"=== {result.scraper_id} v{result.version} ===\n{result.logs}"
                )
    ...
    async def deploy_to_production(
        self, scraper_code: ScraperCode
    ) -> ProductionRelease:
        """Deploy a scraper to production after validation"""

        scraper_id = f"{scraper_code.site_name}_{scraper_code.action_type}"
        release_id = (
            f"{scraper_id}_release_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}"
        )
        ...
        try:
            ...
            final_validation = await validator.validate_scraper(
                scraper_code=scraper_code, min_success_rate=self.min_success_rate
            )

            if not final_validation.overall_success:
                raise Exception(
                    f"Scraper failed final validation: {final_validation.success_rate:.2%}"
                )
            ...
            logger.info(
                f"Successfully deployed {scraper_id} v{scraper_code.version} to production"
            )
    ...
    def _calculate_avg_execution_time(self, validation_run: ValidationRun) -> float:
        ...
        execution_times = [
            result.execution_time for result in validation_run.results if result.success
        ]
    ...
    async def _copy_to_production(self, scraper_code: ScraperCode):
        ...
        prod_metadata = (
            f"{scraper_code.site_name}_{scraper_code.action_type}_metadata.json"
        )

        try:
            # Copy files
            shutil.copy2(
                os.path.join(source_dir, source_file), os.path.join(prod_dir, prod_file)
            )
    ...
    async def _update_site_directory(
        self, scraper_code: ScraperCode, release: ProductionRelease
    ):
        ...
        try:
            ...
            if site_name in site_directory["sites"]:
                ...
                # Update site status
                if all(
                    scraper.get("production_ready", False)
            ...
            site_directory["metadata"]["production_ready_count"] = sum(
                1
                for site in site_directory["sites"].values()
    ...
    async def rollback_production(
        self, scraper_id: str, target_version: Optional[int] = None
    ) -> bool:
        ...
        if target_version is None:
            ...
            for release in reversed(releases[:-1]):
                if (
                    release.deployment_status == DeploymentStatus.PRODUCTION
                    and release.success_rate >= self.min_success_rate
                ):
        ...
        try:
            logger.info(
                f"Rolling back {scraper_id} from v{current_release.version} to v{target_version}"
            )
    ...
    def get_improvement_summary(self, scraper_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "latest_release": {
                ...
                "deployment_status": latest_release.deployment_status.value
                if latest_release
                else None,
                "success_rate": latest_release.success_rate if latest_release else None,
                "deployed_at": latest_release.deployed_at.isoformat()
                if latest_release
                else None,
            }
            if latest_release
    ...
    async def continuous_improvement(
        self, scraper_id: str, interval_hours: int = 168
    ):  # Weekly
        ...
        while True:
            try:
                ...
                if validation_result.success_rate < self.min_success_rate:
                    logger.info(
                        f"Continuous improvement triggered for {scraper_id}: {validation_result.success_rate:.2%}"
                    )

                    # Run improvement process
                    improvement_result = await self.improve_scraper_iteratively(
                        latest_scraper
                    )
                    ...
                    if improvement_result.success:
                        await self.deploy_to_production(
                            improvement_result.generation_result.scraper_code
                        )
