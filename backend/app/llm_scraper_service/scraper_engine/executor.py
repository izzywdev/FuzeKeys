...
class DockerScraperExecutor:
    ...
    async def _prepare_scraper_environment(
        self, temp_dir: str, scraper_code: str, test_data: Dict[str, Any]
    ):
...
""".strip()
    ...
    async def _create_container(self, temp_dir: str, timeout: int):
        ...
        try:
            ...
            image, build_logs = self.docker_client.images.build(
                path=temp_dir, tag=image_tag, rm=True, forcerm=True
            )
    ...
    async def _collect_results(
        self, container, temp_dir: str, execution_logs: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if os.path.exists(result_file):
                ...
                if not result["success"]:
                    result["error_message"] = scraper_result.get(
                        "error", "Unknown error"
                    )
