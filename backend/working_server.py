...
async def startup_event():
    ...
    try:
        ...
        if openai_key == "demo_key":
            print(
                "⚠️  Warning: Using demo OpenAI key. Set OPENAI_API_KEY environment variable for full functionality."
            )
...
async def get_identities(db: AsyncSession = Depends(get_async_session)):
    ...
    try:
        ...
        for identity in identities:
            ...
            first_name = (
                demo_encryption.decrypt(identity.encrypted_first_name)
                if identity.encrypted_first_name
                else ""
            )
            last_name = (
                demo_encryption.decrypt(identity.encrypted_last_name)
                if identity.encrypted_last_name
                else ""
            )
            email = (
                demo_encryption.decrypt(identity.encrypted_email)
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching identities: {str(e)}"
        )
...
async def get_accounts(db: AsyncSession = Depends(get_async_session)):
    ...
    try:
        ...
        for account, identity in account_identity_pairs:
            ...
            username = (
                demo_encryption.decrypt(account.encrypted_username)
                if account.encrypted_username
                else ""
            )
            email = (
                demo_encryption.decrypt(account.encrypted_email)
                if account.encrypted_email
                else ""
            )
            notes = (
                demo_encryption.decrypt(account.encrypted_notes)
                if account.encrypted_notes
                else ""
            )
