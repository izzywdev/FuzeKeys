...
class GoogleFormDebugger:
    ...
    def capture_form_structure(self, page_name):
        ...
        for combo in form_data["comboboxes"]:
            print(
                f"  - '{combo['text']}' (visible: {combo['visible']}, aria-label: '{combo['ariaLabel']}')"
            )
        ...
        for inp in form_data["inputs"][:10]:  # Show first 10
            ...
            print(
                f"  - {inp['type']} '{inp['id']}' (name: '{inp['name']}', {visibility})"
            )
    ...
    def approach_1_hidden_input(self):
        ...
        try:
            ...
            hidden_inputs = self.driver.find_elements(
                By.XPATH,
                "//input[@type='hidden' and (contains(@name, 'gender') or contains(@name, 'Gender'))]",
            )
        ...
        try:
            ...
            if aria_controls:
                ...
                if controlled_element:
                    ...
                    options = controlled_element.find_elements(
    ...
    def verify_form_values(self):
        ...
        for combo in form_values.get("combobox_states", []):
            print(
                f"  Combobox {combo['index']}: '{combo['text']}' (expanded: {combo['aria_expanded']})"
            )
