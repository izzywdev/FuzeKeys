...
class FinalGoogleSignup:
    ...
    def select_dropdown_by_aria_controls(
        self, combobox_selector, target_value, dropdown_type="dropdown"
    ):
        ...
        try:
            ...
            try:
                ...
                print(
                    f"  ⚠️ Could not find listbox by ID, looking for role='listbox'..."
                )
            ...
            for i, option in enumerate(options[:5]):
                ...
                print(
                    f"    Option {i}: text='{option_text}', data-value='{option_data_value}', visible={option_visible}"
                )
                ...
                try:
                    ...
                    if dropdown_type == "month":
                        ...
                        if expected_text in current_text:
                            print(
                                f"  ✅ {dropdown_type} selection verified: {expected_text}"
                            )
                            return True
                        ...
                        if current_text and current_text != "Gender":
                            ...
                            print(
                                f"  ⚠️ {method_name} click executed but selection not verified"
                            )
    ...
    def try_alternative_gender_methods(self):
        ...
        try:
            ...
            hidden_gender_input = self.driver.find_element(
                By.XPATH, '//input[@aria-label="What\'s your gender?"]'
            )
            ...
            gender_combobox = self.driver.find_element(
                By.XPATH, "//div[@role='combobox' and contains(text(), 'Gender')]"
            )
            self.driver.execute_script(
                "arguments[0].textContent = 'Male';", gender_combobox
            )
    ...
    async def complete_google_signup(self):
        ...
        try:
            ...
            if next_button:
                ...
                if (
                    "username" in final_url
                    or "email" in final_url
                    or "account" in final_url
                ):
