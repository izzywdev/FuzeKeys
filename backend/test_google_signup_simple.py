...
class SimpleGoogleSignupTest:
    ...
    async def test_google_signup(self, signup_data):
        ...
        try:
            ...
            try:
                ...
                try:
                    if aria_controls:
                        ...
                        listbox = self.driver.find_element(
                ...
                for option in options:
                    ...
                    if data_value == signup_data["birth_month_value"]:
                        ...
                        print(
                            f"  🎯 Found target month option by data-value: {signup_data['birth_month_value']}"
                        )
                        break
                ...
                if target_option:
                    ...
                    print(
                        f"  ❌ Could not find month option with data-value: {signup_data['birth_month_value']}"
                    )
            ...
            dropdown_x = (
                dropdown_location["x"] + dropdown_size["width"] - 20
            )  # 20px from right edge
            dropdown_y = (
                dropdown_location["y"] + dropdown_size["height"] // 2
            )  # Center vertically
            ...
            data_value_1_options = self.driver.find_elements(
                By.XPATH, "//div[@role='option' and @data-value='1']"
            )
            if data_value_1_options:
                male_option = data_value_1_options[0]  # Use first one found
                print(
                    f"   ✅ Found option with data-value='1' (visible={male_option.is_displayed()})"
                )
            else:
            ...
            if not male_option:
                ...
                male_text_options = self.driver.find_elements(
                    By.XPATH, "//div[@role='option' and contains(text(), 'Male')]"
                )
            ...
            if not male_option:
                ...
                all_gender_options = self.driver.find_elements(
                    By.XPATH, "//div[@role='option' and @data-value]"
                )
                ...
                if gender_candidates:
                    ...
                    print(
                        f"   ✅ Using first gender candidate with data-value='{data_val}'"
                    )
                else:
            ...
            if not male_option:
                ...
                for i, option in enumerate(all_options):
                    try:
                        ...
                        print(
                            f"     Option {i}: text='{text}', data-value='{data_value}', visible={visible}, enabled={enabled}, location={location}"
                        )
                    except Exception as e:
            ...
            try:
                ...
                if "Male" in selected_gender_text:
                    ...
                    print(
                        f"   ⚠️ Gender dropdown text is '{selected_gender_text}', expected 'Male'. Selection might have failed."
                    )
            except Exception as e:
            ...
            personal_details_next_button = wait.until(
                EC.element_to_be_clickable(
                    (By.XPATH, "//button[.//span[text()='Next']]")
                )
            )
            self.driver.execute_script(
                "arguments[0].scrollIntoView(true);", personal_details_next_button
            )
            time.sleep(0.5)
            ...
            print(
                f"❌ An error occurred during the signup test: {e.__class__.__name__}: {e}"
            )
