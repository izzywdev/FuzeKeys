...
def upgrade():
    op.create_table(
        ...
        sa.Column("sensitivity", sa.String(length=20), nullable=False, server_default="medium"),
    ...
    op.create_index("ix_broker_grants_grant_id", "broker_grants", ["grant_id"], unique=True)
    op.create_index("ix_broker_grants_handle_fingerprint", "broker_grants", ["handle_fingerprint"])
    op.create_index("ix_broker_grants_redeemer_identity", "broker_grants", ["redeemer_identity"])
    op.create_index("ix_broker_grants_grantor_identity", "broker_grants", ["grantor_identity"])
