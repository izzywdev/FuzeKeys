...
def _make_user(session, email="john@acme.test", username="john"):
    user = User(
        email=email, username=username, hashed_password="x", master_key_hash="y"
    )
    session.add(user)
    session.commit()
    return user
...
def test_identity_has_org_and_vault_ref(session):
    ...
    session.commit()
    ident = Identity(
        name="Alice Smith", user_id=1, org_id=org.id, vault_collection_ref="col_abc123"
    )
    session.add(ident)
    session.commit()
...
def test_approval_defaults_pending_and_audit_has_no_secret_column(session):
    ...
    entry = AuditLog(
        ...
        on_behalf_user_id=1,
        identity_id=42,
        resource_ref="identity:42:login:twitter",
