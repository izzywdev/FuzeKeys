...
class TestPermitIOIntegration:
    ...
    async def test_signup_account(
        self, mock_signup, sample_credentials, sample_success_result
    ):
    ...
    async def test_signin_account(self, mock_signin, sample_success_result):
        ...
        mock_signin.assert_called_once_with(
            "test@example.com", "password", headless=True
        )
    ...
    async def test_create_api_key(self, mock_apikey, sample_success_result):
        ...
        result = await integration.create_api_key(
            "test@example.com", "password", "TestKey"
        )

        assert result.success is True
        mock_apikey.assert_called_once_with(
            "test@example.com", "password", "TestKey", headless=True
        )
