    ...
    def setup_driver(self):
        ...
        try:
            ...
            try:
                ...
                raise Exception(
                    f"Could not initialize Chrome WebDriver. Original error: {e}"
                )
    ...
    def analyze_page_structure(self, page_name="current"):
        ...
        try:
            ...
            elements = {
                ...
                "listboxes": self.driver.find_elements(
                    By.XPATH, "//div[@role='listbox']"
                ),
                "options": self.driver.find_elements(
                    By.XPATH, "//div[@role='option'] | //li[@role='option']"
                ),
                "clickable_divs": self.driver.find_elements(
                    By.XPATH, "//div[@role='button'] | //div[contains(@class, 'click')]"
                ),
            }
            ...
            for element_type, element_list in elements.items():
                ...
                for i, elem in enumerate(
                    ...
                    try:
                        ...
                        elem_aria_label = (
                            elem.get_attribute("aria-label") or "no-aria-label"
                        )
                        ...
                        if element_type == "options":
                            elem_data_value = (
                                elem.get_attribute("data-value") or "no-data-value"
                            )
                            print(
                                f"  {i}: id='{elem_id}', text='{elem_text}', data-value='{elem_data_value}', visible={elem_visible}"
                            )
                        else:
                            print(
                                f"  {i}: id='{elem_id}', name='{elem_name}', type='{elem_type}', text='{elem_text}', visible={elem_visible}"
                            )
                        ...
                        if (
                            "month" in elem_text.lower()
                            or "gender" in elem_text.lower()
                            or "month" in elem_aria_label.lower()
                            or "gender" in elem_aria_label.lower()
                        ):
                            print(
                                f"      🎯 POTENTIALLY RELEVANT FOR BIRTH DATE/GENDER!"
                            )
        ...
        if target_type == "month":
            strategies = [
                ...
                lambda: self.driver.find_elements(
                    By.XPATH, "//div[@role='combobox' and contains(text(), 'Month')]"
                ),
                # Strategy 2: Look for combobox with month aria-label
                lambda: self.driver.find_elements(
                    By.XPATH,
                    "//div[@role='combobox' and contains(@aria-label, 'Month')]",
                ),
            ...
            strategies = [
                ...
                lambda: self.driver.find_elements(
                    By.XPATH, "//div[@role='combobox' and contains(text(), 'Gender')]"
                ),
                # Strategy 2: Look for combobox with gender aria-label
                lambda: self.driver.find_elements(
                    By.XPATH,
                    "//div[@role='combobox' and contains(@aria-label, 'Gender')]",
                ),
            ...
            strategies = [
                ...
                lambda: self.driver.find_elements(
                    By.XPATH,
                    "//input[contains(@aria-label, 'Day') or contains(@placeholder, 'Day')]",
                ),
            ]
            ...
            strategies = [
                ...
                lambda: self.driver.find_elements(
                    By.XPATH,
                    "//input[contains(@aria-label, 'Year') or contains(@placeholder, 'Year')]",
                ),
            ]
    ...
    def handle_dropdown(self, dropdown_element, dropdown_type, target_value):
        ...
        try:
            ...
            for i, option in enumerate(options[:10]):
                ...
                option_data_value = (
                    option.get_attribute("data-value") or "no-data-value"
                )
                print(
                    f"  Option {i}: text='{option_text}', data-value='{option_data_value}'"
                )
            ...
            if dropdown_type == "month":
                ...
                for option in options:
                    if option.get_attribute("data-value") == target_data_value:
                        target_option = option
                        print(
                            f"🎯 Found gender option by data-value: {target_data_value}"
                        )
                        break
            ...
            if not target_option:
                print(
                    f"❌ Could not find target option for {dropdown_type}: {target_value}"
                )
                ...
                for method in click_methods:
                    try:
                        if method == "regular":
                            ...
                            self.driver.execute_script(
                                "arguments[0].click();", target_option
                            )
                        ...
                        if current_text and current_text not in [
                            ...
                            print(
                                f"✅ {dropdown_type} selection verified: '{current_text}'"
                            )
                            # Close dropdown
                            ...
                            print(
                                f"⚠️ {method} click executed but selection not verified"
                            )
    ...
    async def test_adaptive_signup(self):
        ...
        try:
            ...
            if next_button:
                ...
                if "username" in current_url or any(
                    word in current_url for word in ["email", "gmail", "account"]
                ):
                    ...
                    try:
                        ...
                        print(
                            "📧 No username field found - Google might auto-generate email"
                        )

                    return {
                        "success": True,
                        "status": "reached_email_setup",
                        "url": current_url,
                    }
                else:
